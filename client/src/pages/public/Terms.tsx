  ...
  const sectionsID: Section[] = [
    ...
    },
    {
      ...
      items: [
        ...
        "Melihat jadwal rilis chapter berikutnya dengan countdown waktu tersisa secara real-time di halaman detail novel",
        "Membuka chapter premium menggunakan Koin WOOCE yang telah diperoleh dari login harian, quest, atau top-up",
    ...
    },
    {
      ...
      paragraphs: [
        "Pengguna dapat mengajukan penghapusan akun mandiri melalui halaman Pengaturan di profil. Proses ini memerlukan verifikasi OTP (One-Time Password) yang dikirim ke email terdaftar untuk memastikan keamanan dan mencegah penghapusan yang tidak disengaja.",
        "Setelah penghapusan dikonfirmasi, seluruh data yang terhubung ke akunmu akan dihapus secara permanen dan menyeluruh dari sistem kami, meliputi:",
      ],
      items: [
        "Data akun dan profil (nama, email, foto profil, role, status)",
        "Profil penulis beserta seluruh novel, season, dan chapter yang ditulis (khusus akun penulis)",
        "Riwayat transaksi koin (perolehan, pengeluaran, bonus, top-up)",
        "Chapter premium yang pernah di-unlock",
        "Data login bonus dan streak harian",
        "Seluruh notifikasi in-app",
        "Riwayat pesanan top-up koin",
        "Data follow terhadap novel atau cerita",
        "Permohonan verifikasi penulis yang belum diproses",
        "Laporan konten yang pernah diajukan",
        "File yang diunggah (foto profil, cover karakter) melalui sistem GridFS",
        "Sesi login aktif",
      ],
      note: "Untuk akun penulis: sistem secara otomatis menghasilkan dan mengirimkan file PDF backup berisi semua karya yang pernah ditulis ke email terdaftar sebelum penghapusan dilakukan. Demikian pula, saat penulis memilih untuk menghapus satu novel atau karya tertentu, sistem akan mengirimkan backup PDF khusus untuk karya tersebut. Admin juga berhak menghapus akun pengguna atau penulis yang melanggar ketentuan — dalam kasus ini, email pemberitahuan dan backup karya (jika berlaku) tetap dikirimkan.",
    },
    {
  ...
  const sectionsEN: Section[] = [
    ...
    },
    {
      ...
      items: [
        ...
        "Viewing the next chapter release schedule with a real-time countdown on the novel detail page",
        "Unlocking premium chapters using WOOCE Coins earned from daily login, quests, or top-up",
    ...
    },
    {
      ...
      paragraphs: [
        "Users may request self-deletion of their account through the Settings tab on their profile page. This process requires OTP (One-Time Password) verification sent to the registered email to ensure security and prevent accidental deletion.",
        "After deletion is confirmed, all data linked to your account will be permanently and comprehensively deleted from our system, including:",
      ],
      items: [
        "Account and profile data (name, email, profile photo, role, status)",
        "Writer profile along with all novels, seasons, and chapters written (writer accounts only)",
        "Coin transaction history (earnings, spending, bonuses, top-ups)",
        "Premium chapters that were unlocked",
        "Daily login bonus data and streaks",
        "All in-app notifications",
        "Coin top-up order history",
        "Novel or story follow data",
        "Pending writer verification requests",
        "Content reports previously submitted",
        "Uploaded files (profile photo, character covers) via the GridFS system",
        "Active login sessions",
      ],
      note: "For writer accounts: the system automatically generates and sends a PDF backup file containing all works ever written to the registered email before deletion. Similarly, when a writer deletes a specific novel, a dedicated PDF backup is sent for that work. Admins also have the right to delete accounts that violate the terms — in such cases, notification emails and work backups (where applicable) are still sent.",
    },
    {
